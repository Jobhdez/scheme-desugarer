...
(deftest test-parse-let ()
  ...
  (is (equalp (parse-exp '(let ((n 3)) (+ n 3)))
...
(deftest test-parse-letrec ()
  ...
  (is (equal  (parse-exp '(letrec ((n (lambda (x) (+ x 1)))) (n 4)))
	      (manifold-scheme:make-letrecscm
	       :bindings (list (manifold-scheme:make-var :v 'n)
...
(deftest test-parse-begin ()
  ...
  (is (equalp (parse-exp '(begin (set! d 3) (set! g 4) (+ d g)))
...
(deftest test-parse-if ()
  ...
  (is (equalp (parse-exp '(if (= 1 1) 1 2))
...
(deftest test-parse-lambda ()
  ...
  (is (equalp (parse-exp '(lambda (n) (* n n)))
